...
class Pieces:
    def __init__(self):
        ...
        self.loc = [[126 + self.wd * 0, 156 + self.wd * 0, 1], [126 + self.wd * 1, 156 + self.wd * 0, 2],
                    [126 + self.wd * 2, 156 + self.wd * 0, 3], [126 + self.wd * 0, 156 + self.wd * 1, 4],
                    [126 + self.wd * 1, 156 + self.wd * 1, 5], [126 + self.wd * 2, 156 + self.wd * 1, 6],
    ...
    def check_solved(self):
        if self.solved_loc == self.loc:
            messagebox.showinfo(
                "Solved",
                f"Congratulations! You have solved the puzzle in {end_time} seconds and {steps} steps")
            sys.exit()
        else:
            return True
    ...
    def search_and_shift(self, pos):
        ...
        for i in range(len(self.loc)):
            if pos[0] > self.loc[i][0] and pos[0] < self.loc[i][0] + \
                    80 and pos[1] > self.loc[i][1] and pos[1] < self.loc[i][1] + 80:
                pressed = True
                break
        if pressed:
            global steps
...
class Puzzle:
    ...
    def draw(self, win):
        ...
        self.sho = str(tp)
        end_time = self.sho
        t = font.render(f"Time: {self.sho} seconds", True, (200, 89, 200))
        g = font.render(f"Step {steps}", True, (200, 89, 200))
        ...
        for i in range(4):
            pygame.draw.line(win, (200, 200, 200), (self.x + 90 * i, self.y),
                             (self.x + 90 * i, self.y + 3 * 90), 10)
...
def before_start(win):
    ...
    g = prefont.render("Made By: Saeid Rasoli", True, (250, 89, 170))
    win.blit(p, (135, 100))
    win.blit(q, (90, 170))
    win.blit(g, (105, 270))
    pygame.display.update()
